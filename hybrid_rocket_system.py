...
class Methane(Fuel):
    def __init__(self):
        ...
        self.specific_heat = 2600.0; self.density = 422.0
...
class Pump(Component):
    ...
    def power(self) -> Float:
        ...
        if self.pressure_gain and self.mass_flow_rate and self.fluid_density and self.efficiency:
            volumetric_flow = self.mass_flow_rate / self.fluid_density
            return self.pressure_gain * volumetric_flow / self.efficiency
        return None
...
class Turbine(Component):
    """Models a turbine driven by heated propellant (expander cycle)"""
    efficiency: Float = None
    mass_flow_rate: Float = None
    temp_in: Float = None
    temp_out: Float = None
    ...
    def power_in(self) -> Float:
        if self.mass_flow_rate and self.specific_heat and self.temp_in and self.temp_out:
            return self.mass_flow_rate * self.specific_heat * (self.temp_out - self.temp_in)
        return None

    @property
    def power_out(self) -> Float:
        p_in = self.power_in
        if p_in and self.efficiency:
            return p_in * self.efficiency
        return None

@dataclass()
class Generator(Component):
    """Models an electrical generator"""
    efficiency: Float = None
    specific_power: Float = None  # kW/kg

@dataclass()
class ElectricMotor(Component):
    """Models an electric motor to drive a pump"""
    efficiency: Float = None
    specific_power: Float = None  # kW/kg

@dataclass()
class Battery(Component):
    """Models a startup battery"""
    energy_capacity: Float = None  # Joules
    specific_energy: Float = None  # J/kg
...
class ConstraintSystem:
    ...
    def solve_system(self) -> Dict[str, float]:
        ...
        try:
            ...
            if sol:
                # Handle single solution dictionary or list of dictionaries
                solution = sol[0] if isinstance(sol, list) else sol
...
class PropulsionSystem:
    ...
    def _setup(self):
        ...
        for name, comp in self.components.items():
            ...
            # Sum pump power
            if isinstance(comp, Pump) and comp.power:
                total_pump_power += comp.power

            # Link motor to pump power
            if isinstance(comp, ElectricMotor):
                # Power required by motor, accounting for efficiency
                total_motor_power_req += total_pump_power / comp.efficiency if comp.efficiency else total_pump_power
                if comp.specific_power:
                    # Mass is derived from power
                    motor_mass = (total_pump_power / 1000) / comp.specific_power
                    cs.add_variable(f"{name}_mass", motor_mass)

            # Link turbine to generator
            if isinstance(comp, Generator):
                # Power required by generator, accounting for efficiency
                gen_power_req = total_motor_power_req / comp.efficiency if comp.efficiency else total_motor_power_req
                if comp.specific_power:
                    # Mass is derived from power
                    gen_mass = (gen_power_req / 1000) / comp.specific_power
                    cs.add_variable(f"{name}_mass", gen_mass)
    ...
    def print_summary(self):
        ...
        # Determine target thrust from chamber if possible
        chamber = next((c for c in self.components.values() if isinstance(c, Chamber)), None)
        if chamber and 'thrust' in vals:
            print(f"Target Thrust: {vals.get('thrust', 0):.0f} N")
        else:
            print(f"Target Thrust: 25,000 N") # Default
        ...
        for name, comp in self.components.items():
            if isinstance(comp, Pump):
                ...
                if power_val:
                    print(f"{name.title()} Power Req: {power_val/1000:.1f} kW")
            if isinstance(comp, Turbine):
                power_val = comp.power_out
                if power_val:
                    print(f"{name.title()} Power Gen: {power_val/1000:.1f} kW")
...
def create_hybrid_expander_rocket() -> PropulsionSystem:
    """Create a 50kN hybrid expander-electric rocket from the template"""
    ...
    # Pumps
    fuel_pump = Pump(
        pressure_gain=delta_P,
        efficiency=0.7,
        mass_flow_rate=mdot_fuel,
        fluid_density=fuel.density
    )
    oxidizer_pump = Pump(
        pressure_gain=delta_P,
        efficiency=0.7,
        mass_flow_rate=mdot_ox,
        fluid_density=oxidizer.density
    )
    ...
    turbine = Turbine(
        ...
        temp_in=300, # K
        temp_out=800, # K
        specific_heat=fuel.specific_heat
    )
    generator = Generator(efficiency=0.9, specific_power=5.0) # kW/kg
    motor = ElectricMotor(efficiency=0.9, specific_power=3.0) # kW/kg

    # Battery (for startup)
    P_pump_total = fuel_pump.power + oxidizer_pump.power
    P_required_elec = P_pump_total / (motor.efficiency * generator.efficiency)
    E_battery = P_required_elec * 10 # 10s startup
    battery = Battery(
        energy_capacity=E_battery,
        specific_energy=720e3, # J/kg (200 Wh/kg)
    )
    battery.mass = E_battery / battery.specific_energy

    # Other components
    chamber = Chamber(
        pressure=70e5, # ~70 bar
        temperature=fuel.combustion_temperature,
        mass_flow_rate=mdot_total,
        mass=150 # Estimate
    )
    nozzle = Nozzle(exit_pressure=101325, mass=100) # Estimate

    # Assemble system
    rocket = PropulsionSystem()
    rocket.add_component("fuel_pump", fuel_pump)
    rocket.add_component("oxidizer_pump", oxidizer_pump)
    rocket.add_component("turbine", turbine)
    rocket.add_component("generator", generator)
    rocket.add_component("motor", motor)
    rocket.add_component("battery", battery)
    rocket.add_component("chamber", chamber)
    rocket.add_component("nozzle", nozzle)
...
def create_test_suite():
    ...
    constraints_25k = rocket_25k.print_summary()

    # Test 2: 50kN Hybrid Expander Rocket
    print("\n\nTEST 2: 50kN Methane/LOX Hybrid Expander-Electric Rocket")
    print("-" * 60)
    hybrid_rocket = create_hybrid_expander_rocket()
    # We need to manually trigger a solve before printing summary for this one
    # because some masses are calculated during setup.
    hybrid_rocket.solve()
    constraints_hybrid = hybrid_rocket.print_summary()


    # Test 3: Component constraint verification
    print("\n\nTEST 3: Constraint System Verification")
    print("-" * 45)
    vals = rocket_25k.solve()
    print("Key Performance Variables (25kN Rocket):")
    for key in ['thrust', 'specific_impulse', 'exit_velocity', 'chamber_pressure', 'total_mass_flow_rate']:
        if key in vals:
            print(f"  {key}: {vals[key]:.2f}")

    # Test 4: Elegant constraint demonstration
    print("\n\nTEST 4: Constraint System Demo")
    print("-" * 45)
    demonstrate_constraints()

    print("\n\nSUMMARY:")
    vals_25k = rocket_25k.solve()
    vals_hybrid = hybrid_rocket.solve()
    print(f"• 25kN Rocket: Achieved {vals_25k['thrust']:.0f}N thrust, Validated {sum(constraints_25k.values())}/{len(constraints_25k)} constraints")
    print(f"• 50kN Hybrid Rocket: Achieved {vals_hybrid.get('thrust', 0):.0f}N thrust, Total Mass: {hybrid_rocket.total_mass():.1f} kg")


    return rocket_25k, hybrid_rocket
